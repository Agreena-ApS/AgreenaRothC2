...
#' @param calibrated Use calibration factors: TRUE or FALSE
...
AgreenaProgramme2 <- function(
  ...
  years <- round(
    as.numeric(as.Date(sim_period[2]) - as.Date(sim_period[1])) / 365
  )
  ...
  ft_s <- SoilR::fT.RothC(t(wth[, "TS_AV"]))
  fw_s <- f_moist_rothc(
    pp = t(wth[, "PRECTOTCORR_AV"]),
    et = t(wth[, "EVPTRNS_AV"]),
    s_thick = 30,
  ...
  run_RothC <- function(x,
    ...
    fxi_calib <- data.frame(
      spin_period,
      rep(x[, "baseline"], length.out = length(spin_period))
    )
    ...
    fxi_b <- data.frame(
      sim_period,
      rep(x[, "baseline"], length.out = length(sim_period))
    )
