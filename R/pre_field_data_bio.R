...
pre_field_data_bio <- function(field_data) {
  ...
  sel_col <- c(
    ...
    "field_def_cover_crops_adoption_logic_app",
    "actual_cover_crops_adoption_logic_app",
    "field_def_tilling",
    "actual_tilling",
    "field_def_crop_residue_management_logic_app",
    "actual_crop_residue_management_logic_app",
  ...
  field_data <- field_data %>%
    ...
    mutate(add_bio_inpts = if_else(
      field_def_crop_residue_management_logic_app %in% c("Removed", "Burned") &
        actual_crop_residue_management_logic_app %in% c("Mulched"), 1,
      if_else(
        field_def_crop_residue_management_logic_app %in% c("Mulched") &
          actual_crop_residue_management_logic_app %in% c("Removed", "Burned"), -1,
        0
      )
    ) * actual_residues) %>%
